...
phase1DesignEX1 <- local({
    Ani = 1:nAni
    if(nAni > 14){
      Trt = (1:nTrt)[c(1,2,1,2,1,2,1,2,2,1,2,1,2,1,2,1)]
    } else {
      Trt = (1:nTrt)[c(1,2,1,2)]
    }
...
for(j in 1:10){
  ...
  for(i in 1:10){ 
    ...
    res <- optim(newInit, obj.fun.new, swap, method="SANN",
        control =
          list(maxit = 1e+05, temp = y2 , tmax = 1000, trace = TRUE, 
                                                         REPORT = 100, fnscale = -1))
    
  


    #12.5 / log(((1e+06-1) %/% 1000)*1000 + exp(1))
...
z1.aveEff = apply(indList, 1, function(x) test(X.trt = Z1.mat[x,],  mI(n) - Pb, Rep=Z1.rep)$ave.eff)

apply(indList, 1, function(x) test(X.trt = Z1.mat[x,],  mI(n) - Pb, Rep=Z1.rep)$can.eff)
