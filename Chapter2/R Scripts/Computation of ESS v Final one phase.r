...
function(design.df, random.terms, fixed.terms, var.comp = NA, trt.contr = NA, table.legend = FALSE){
  ...
  #random.terms = a single string of characters contains the tier structrue for the random factors  using the W-R syntax. 
...
print("Pre- and post-multiply NTginvATN by block projection matrices.")  
  ...
  #pre- and post-multiply NTginvATN by block projection matrices  
   PNTginvATNP <- lapply(Pb,function(z) blkProkMat(z, T, N, Rep))
...
print("Finally get coefficients for each treatment effect in each stratum.")
    ...
    for(i in 1:length(PNTginvATNP)){
      tmp <- matrix(0, nrow=length(names(PNTginvATNP[[i]])) - 1, ncol=length(V), 
                    dimnames=list(names(PNTginvATNP[[i]])[-length(names(PNTginvATNP[[i]]))], names(V)))        
      for(j in 1:(length(names(PNTginvATNP[[i]])) - 1)){
        for(z in 1:(length(V))){
      ...
      trt.temp =  attr(fractions(c(trt.Coef*PNTginvATNP[[i]][[length(names(PNTginvATNP[[i]]))]],
                    PNTginvATNP[[i]][[length(names(PNTginvATNP[[i]]))]])),"fracs") 
      trt = rbind(trt,trt.temp)
      if( names(PNTginvATNP[i]) =="Within"){
         rownames(trt)[nrow(trt)] = "Within"
      }else{
        rownames(trt)[nrow(trt)] =  paste("Between", names(PNTginvATNP[i]), sep = ".")
      }  
...
design.df = design
random.terms ="(Row*(Squ/Col))/Hal"
fixed.terms = "Tre*Met"
var.comp = NA
trt.contr = NA
  
VCs = getVCs.onePhase(design.df, random.terms = random.terms,fixed.terms=fixed.terms, var.comp = NA, trt.contr = NA, table.legend = FALSE)
