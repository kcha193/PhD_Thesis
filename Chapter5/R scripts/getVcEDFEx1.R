...
getVcEDFEx1 = 
  function(aov.table, MS = NA, row.MS = NA, true.VC = rep(NA, ncol(aov.table$A) - 2), 
    ...
    LC.var.comp <- matrix(0, nrow = 3, ncol = 1)
    LC.var.comp[1,1] <-  LC.VC.numeric[1, "MS"]
    LC.var.comp[2,1] <-  (LC.VC.numeric[2, "MS"] -  LC.VC.numeric[1, "MS"])/(2)
    LC.var.comp[3,1] <- (LC.VC.numeric[3, "MS"] -  LC.VC.numeric[1, "MS"])/4
    ...
    while (!all(apply(conv, 1, function(x) 
      isTRUE(all.equal(x, conv[nrow(conv), ], tolerance = 1e-07))))) {
    ...
    for (i in 1:nrow(G.mat)) {
      ...
      LC.EDF[i] <- DFF[i] + (temp[3]/DFF[i])/(temp[1]/DFF[i] + temp[2]/(DFF[i] + 1) + temp[4]/DFF[i])
