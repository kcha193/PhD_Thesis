...
edf.df <- local(
{
   edf <- rbind(stack(data.frame(t(outpmds[-2,]))), # Eliminate case sa2/s2=0.01
                stack(data.frame(t(outp[-2,]))),
                stack(data.frame(t(outplc[-2,]))))
   edf.mat <- as.matrix(edf)
   edf$spR <- rep(c(10^((-8:8)/2)), times=17)
...
#   edf$saR <- factor(gsub("X",expression(paste(sigma^2[alpha]," / ", sigma^2, " = "), edf.mat[,"ind"]),
   ...
   edf$Method <- factor(rep(1:3,times=c(102,102,85)),
