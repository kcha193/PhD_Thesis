...
##############################################################################################
optBIBD = function(nTrt, bRep, nCag, tRep, nPlot, iter) {
    n = nTrt * bRep * tRep
    nBlk = n/nPlot
    trt.rep = n/nTrt
...
bibd = seq(1,nrow(phase1DesignEX1),nCag+1)

print(summary.aov.onePhase(phase1DesignEX1[-bibd,], blk.str = "Cag/Ani", trt.str = "Trt"))
...
twoStageBIBD = function(init, iter = 50000, obj.fun = obj.fun.new) {
   print(c(nTrt, bRep, nCag, tRep, nPlot))
    ...
    temp = initialTemp(iter, newInit, swap.new, obj.fun)
    ...
    res1 <- optim(res1$par, obj.fun, swap.new, method = "SANN", control = list(maxit = iter, temp = y2,
        tmax = 1000, trace = TRUE, REPORT = 10, fnscale = -1))
...
newInit = c(1:n, 1)
temp1 = initialTemp(iter, newInit, swap.stage2.new, obj.fun.new)$newInit
temp2 = initialTemp(iter, newInit, swap.stage1.new, obj.fun.new)$newInit
temp3 = initialTemp(iter, newInit, swap.new, obj.fun.new)$newInit
test.newInit = apply(rbind(temp1,temp2,temp3),1,obj.fun.new)
newInit = rbind(temp1,temp2,temp3)[which(test.newInit == max(test.newInit))[1],]

print(test.obj.fun.new(newInit))


newResDF = test.obj.fun.new(newInit)$res
...
bestInd = twoStageBIBD(init = bestInd, iter = 1000, obj.fun = obj.fun.new1)
...
design.df = optBIBD(6,6,6,2,4, 1000)

design.df = optBIBD(4,4,4,2,4, 1000)
