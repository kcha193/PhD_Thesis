...
try.obj.fun.RBD = function(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, blk.proj, nTrt, C.cage, cage.Rep, C.ani, ani.Rep) {
...
obj.fun.RBD = function(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, blk.proj, nTrt, C.cage, cage.Rep, C.ani, ani.Rep) {

  ans = try(try.obj.fun.new(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, 
            blk.proj, nTrt, C.cage, cage.Rep, C.ani, ani.Rep), silent = TRUE)
  
  ifelse(class(ans) == "try-error", 0, ans)
}

test.obj.fun.RBD = function(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, blk.proj, nTrt, C.cage, cage.Rep, C.ani, ani.Rep) {
...
threeStageRBD = function(init, iter = 50000, obj.fun = obj.fun.new) {
    ...
    res1 <- optim(res$par, obj.fun, swap.stage1.new, method = "SANN", 
                  control = list(maxit = iter, temp = y2.stage1,
                  tmax = iter/10, trace = TRUE, REPORT = 10, fnscale = -1))

    res1 <- optim(res1$par, obj.fun, swap.stage3.new, method = "SANN", 
                  control = list(maxit = iter, temp = y2,
                  tmax = iter/10, trace = TRUE, REPORT = 10, fnscale = -1))
    ...
    if(isTRUE(all.equal(cur, 1))) return(res1$par[-length(res1$par)])

    while ((cur - old) > 1e-06) {
        res1 <- optim(res1$par, obj.fun, swap.stage2.new, method = "SANN", 
                      control = list(maxit = iter, temp = y2.stage2,
                      tmax = iter/10, trace = TRUE, REPORT = 10, fnscale = -1))

        res1 <- optim(res1$par, obj.fun, swap.stage1.new, method = "SANN", 
                      control = list(maxit = iter, temp = y2.stage1,
                      tmax = iter/10, trace = TRUE, REPORT = 10, fnscale = -1))
...
#With Example nTrt = 2 bRep = 3 nCag = 3 tRep = 2
...
#With Example nTrt = 3 bRep = 4 nCag = 2 tRep = 2
...
bestInd2 = threeStageRBD(init =  1:n, iter = 1000, obj.fun = obj.fun.old2)

quickTest(bestInd2)
################################################################################
#Objective function section comparesion 3
#With Example nTrt = 4 bRep = 2 nCag = 2 tRep = 2  nPlot = 4
print(obj.fun.old2(c(1:n,1)))

bestInd1 = threeStageRBD(init =  1:n, iter = 1000, obj.fun = obj.fun.old2)
...
bestInd1 = threeStageRBD(init =  bestInd1, iter = 1000, obj.fun = obj.fun.new)


bestInd1 = threeStageRBD.spe(init =  1:n, iter = 1000, obj.fun = obj.fun.new)
bestInd1 = threeStageRBD.spe(init =  bestInd1, iter = 1000, obj.fun = obj.fun.new)
...
old = proc.time()
#bestInd1 = 1:n 
bestInd2 = threeStageRBD(init = bestInd1, iter = 10000, obj.fun = obj.fun.new1)
proc.time() - old 
...
old = proc.time()
#bestInd1 = 1:n 
bestInd2 = threeStageRBD(init = bestInd2, iter = 1000, obj.fun = obj.fun.new1)
proc.time() - old 

old = proc.time()
bestInd1 = 1:n 
bestInd2 = threeStageRBD.spe(init = bestInd1, iter = 10000, obj.fun = obj.fun.new1)
proc.time() - old 

old = proc.time()
#bestInd1 = 1:n 
bestInd2 = threeStageRBD.spe(init = bestInd2, iter = 10000, obj.fun = obj.fun.new1)
proc.time() - old 
...
obj.fun.RBD1 = function(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, blk.proj, nTrt, C.cage, cage.Rep, C.ani, ani.Rep, newResDF) {
  ...
  ans = try(try.obj.fun.new1(ind, Z1.mat, X.trt, nPlot, Z1.rep, trt.rep, 
...
old = proc.time()
bestInd2 = threeStageRBD(init = bestInd2, iter = 10000, obj.fun = obj.fun.new1)
proc.time() - old 
...
levels(design.df$Ani) = LETTERS[rep(1:(nlevels(design.df$Ani)/nCag), nCag)]

summaryAovOnePhase(design.df,  blk.str = "Cag/Ani", trt.str = "Trt")
...
summaryAovOnePhase(design.df,  blk.str = "Cag/Ani", trt.str = "Trt", latex = TRUE, fixed.name = "\\tau")
...
matrix(paste(as.numeric(design.df$Cag), design.df$Ani, sep = ""), 
...
(N = with(design.df, table(Ani,Cag)))
N %*% t(N)
...
trt.str = "Tag + Trt",trt.contr = list(Tag = NA,
                                       Trt = list(  "1" = trt.contr1,
                                                    ...
                                                    "4" = trt.contr4,
                                                    "5" = trt.contr5)),
              latex = TRUE, fixed.names = c("\\gamma", "\\tau_1", "\\tau_2",
                                            "\\tau_3", "\\tau_4", "\\tau_5"))
